...
def gerar_pdf_pedido(o, cli):
    ...
    c.drawString(50, y, 'TIGRÃO DISTRIBUIDORA')
    y -= 30
    c.setFont('Helvetica-Bold', 13)
    c.drawString(50, y, f"Pedido #{o.get('orderNumber','')}")
    y -= 22
    c.setFont('Helvetica', 10)
    linhas = [
        f"Cliente: {o.get('clientName','')}",
        f"CNPJ/CPF: {cli.get('document','') if cli else ''}",
        f"Vendedor: {o.get('salespersonName','')}",
        f"Data: {o.get('date','')}",
        f"Prazo: {o.get('paymentTerm','')}",
    ]
    for linha in linhas:
        c.drawString(50, y, linha[:95]); y -= 16
    y -= 10
    c.setFont('Helvetica-Bold', 10)
    c.drawString(50, y, 'ITENS DO PEDIDO')
    y -= 18
    c.setFont('Helvetica', 10)
    for it in o.get('items', []):
        qtd = float(it.get('quantity',0)); preco = float(it.get('price',0)); total = qtd * preco
        nome = str(it.get('productName',''))[:68]
        if y < 80:
    ...
    y -= 5
    c.setFont('Helvetica-Bold', 13)
    c.drawString(50, y, f"TOTAL: {money(o.get('total',0))}")
    c.save()
    out.seek(0)
    return out.getvalue()
...
def orders_page(db):
    ...
    for o in sorted(filtered_orders(db), key=lambda x:x.get('orderNumber',0), reverse=True):
        ...
        with st.expander(f"Ver detalhes do pedido #{o.get('orderNumber','')}"):
            ...
            st.download_button('⬇️ Baixar PDF', data=pdf, file_name=f"pedido_{o.get('orderNumber','')}.pdf", mime='application/pdf', key='baixar_pdf_'+o['id'])
