...
def dashboard(db):
 ...
 botoes=[
  ...
  ('PEDIDOS FATURADOS','orders'),
  ('VENDAS POR CLIENTE','clients'),
  ('COMISSÃO','more'),
...
POR PRODUTO','more'),
  ...
  ('DEVOLUÇÕES','orders'),
  ('VENDAS POR DIVISÃO','more'),
 ]
 with st.container(key='consulta_grid'):
  for i in range(0,len(botoes),2):
   c1,c2=st.columns(2)
   nome,tela=botoes[i]
   if c1.button(nome,key=f'consulta_btn_{i}'):
    st.session_state.tab=tela
    st.rerun()
   if i+1 < len(botoes):
    nome2,tela2=botoes[i+1]
    if c2.button(nome2,key=f'consulta_btn_{i+1}'):
     st.session_state.tab=tela2
     st.rerun()
