...
INITIAL_ORDERS = [
    {"id": "ord-0", "orderNumber": 0, "date": "2026-06-30T17:34:36", "salespersonId": "sales-1", "salespersonName": "Vendedor", "clientId": "cli-1", "clientName": "nelson das galaxie", "items": [{"productId": "prod-8", "productName": "Whisky Johnnie Walker Red Label 1L", "productCode": "JWL-008", "quantity": 20, "price": 99.90}, {"productId": "prod-7", "productName": "Vodka Smirnoff 998ml", "productCode": "SMI-007", "quantity": 30, "price": 39.90}], "total": 3195.00, "paymentTerm": "BOLETO - 30 DD", "commissionRate": 7, "commissionAmount": 223.65, "status": "Faturado"},
]
...
def buscar_cnpj_internet(cnpj):
    ...
    try:
        ...
        return {"document": cnpj_limpo, "name": d.get("nome_fantasia") or d.get("razao_social") or "", "email": d.get("email") or "", "phone": telefone, "city": d.get("municipio") or "", "state": d.get("uf") or "", "address": d.get("logradouro") or "", "district": d.get("bairro") or "", "zip": d.get("cep") or ""}, None
    except Exception:
...
def gerar_pdf_pedido(o, cli):
    ...
    fantasia = str(cli.get("fantasy", "") or cli.get("name", "") or o.get("clientName", "")).upper() if cli else cliente_nome
    endereco = str(cli.get("address", "") if cli else "").upper()
    bairro = str(cli.get("district", "") if cli else "").upper()
    ...
    draw_text(c, 420, y, "Pagina:", 10); draw_text(c, 485, y, "1", 10); draw_text(c, 515, y, "/", 10); draw_text(c, 540, y, "1", 10)
    y -= 14
    draw_text(c, 100, y, "R JERÔNIMO DE LEMOS, 339 - VILA ISABEL.  RIO DE JANEIRO - RJ", 7)
    draw_text(c, 335, y, "21967208370", 7)
    y -= 8; line(c, left, y, right, y, 1.1)
    y -= 14; draw_text(c, 225, y, "***DOCUMENTO SEM VALOR FISCAL***", 9, True)
    y -= 18
    ...
    y -= 10; line(c, left, y, right, y, 1.1)
    y -= 17
    ...
    y -= 6; line(c, left, y, right, y, .8); y -= 12

    for it in o.get("items", []):
        qtd = float(it.get("quantity", 0)); preco = float(it.get("price", 0))
    ...
    y = 250
    c.setDash(3, 2); line(c, left, y, 265, y, .8); line(c, 355, y, right, y, .8); c.setDash()
    ...
    y -= 14; line(c, left, y, right, y, .8); y -= 20
    ...
    y -= 10; line(c, left, y, right, y, .8); y -= 18
    ...
    y -= 60; line(c, 330, y, right - 5, y, .8); y -= 14
...
def pdf_preview(pdf_bytes, height=760):
    ...
    components.html(f'''<iframe src="data:application/pdf;base64,{b64}#toolbar=0&navpanes=0&scrollbar=1" width="100%" height="{height}" style="border:1px solid #eee;border-radius:14px;background:white;"></iframe>''', height=height + 20)


def link_whatsapp_pedido(o, cli):
    pedido_num = f"{int(o.get('orderNumber', 0)):06d}"
    texto = f"Olá! Segue o pedido #{pedido_num} da Tigrão Distribuidora.\nCliente: {o.get('clientName', '')}\nTotal: {money(o.get('total', 0))}\n\nVou anexar o PDF do pedido nesta conversa."
...
def mostrar_aviso():
    ...
    if not msg:
        return
    components.html(f'''
    <div style="position:fixed;top:22px;right:18px;z-index:999999999;background:white;border:3px solid #fb923c;border-radius:22px;padding:12px 16px;box-shadow:0 10px 30px rgba(0,0,0,.25);font-family:Arial,sans-serif;animation:toastInOut 1.4s ease-in-out forwards;">
      <div style="font-size:15px;font-weight:900;color:#111;">🐯 {msg}</div>
    </div>
    <script>try{{navigator.vibrate && navigator.vibrate(45);}}catch(e){{}}</script>
    <style>@keyframes toastInOut{{0%{{opacity:0;transform:translateY(-18px) scale(.92)}}15%{{opacity:1;transform:translateY(0) scale(1)}}80%{{opacity:1}}100%{{opacity:0;transform:translateY(-18px) scale(.92)}}}}</style>
    ''', height=0)
...
def css():
 ...
 .stButton>button:active,.stDownloadButton>button:active,.stLinkButton>a:active{transform:scale(.94)!important;filter:brightness(1.18)!important;box-shadow:0 0 0 5px rgba(249,115,22,.25)!important;animation:comandoPulse .35s ease-in-out!important}.stButton>button:focus,.stDownloadButton>button:focus,.stLinkButton>a:focus{outline:3px solid rgba(249,115,22,.45)!important}@keyframes comandoPulse{0%{transform:scale(.94)}50%{transform:scale(1.03)}100%{transform:scale(1)}}
...
def tigrinho_salvo():
    if not st.session_state.get("show_tigrinho_salvo"):
        return
    components.html('''<div id="tigrinho" style="position:fixed;top:28px;right:22px;z-index:999999999;background:white;border:3px solid #fb923c;border-radius:24px;padding:12px 16px;box-shadow:0 10px 35px rgba(0,0,0,.25);text-align:center;font-family:Arial,sans-serif;animation:entraSai 1.8s ease-in-out forwards;"><div style="font-size:58px;line-height:1">🐯</div><div style="font-size:15px;font-weight:900;color:#111;margin-top:4px">Pedido salvo!</div></div><script>try{navigator.vibrate && navigator.vibrate([40,30,40]);}catch(e){}</script><style>@keyframes entraSai{0%{opacity:0;transform:translateY(-25px) scale(.8)}15%{opacity:1;transform:translateY(0) scale(1)}75%{opacity:1;transform:translateY(0) scale(1)}100%{opacity:0;transform:translateY(-25px) scale(.8)}}</style>''', height=0)
...
def login(db):
    ...
    if ok:
        ...
        if usuario_limpo in ["admin", "administrador"] and senha == "admin123":
            aviso("Entrada autorizada")
            st.session_state.auth = True; st.session_state.role = "admin"; st.session_state.sales_id = ""; st.session_state.user_name = "Administrador"
            st.rerun()
        ...
        if seller:
            aviso("Entrada autorizada")
            st.session_state.auth = True; st.session_state.role = "vendedor"; st.session_state.sales_id = seller["id"]; st.session_state.user_name = seller["name"]
            st.rerun()
        ...
        with st.expander(f"Ver detalhes do pedido #{o.get('orderNumber', '')}"):
            ...
            if st.session_state.role == "admin":
                ...
                if col1.button("Atualizar", key="up" + o["id"]):
                    o["status"] = ns; save_db(db); aviso("Status atualizado"); st.rerun()
                if col2.button("Excluir pedido", key="del" + o["id"]):
                    aviso("Confirme a exclusão")
                    st.session_state["pedido_para_excluir"] = o["id"]; st.rerun()
            ...
            with st.form("prod"):
                ...
                if st.form_submit_button("Salvar produto") and n:
                    if produto_duplicado(db, n):
                        ...
                        save_db(db); aviso("Produto salvo"); st.rerun()
            ...
            if st.session_state.role == "admin":
                ...
                if excluir:
                    ...
                    if usado:
                        ...
                        save_db(db); aviso("Produto excluído"); st.rerun()
                if editar or st.session_state.get("editando_produto") == p["id"]:
                    ...
                    if salvar:
                        ...
                        if repetido:
                            ...
                            save_db(db); st.session_state.pop("editando_produto", None); aviso("Produto atualizado"); st.rerun()
                    if cancelar:
                        st.session_state.pop("editando_produto", None); aviso("Edição cancelada"); st.rerun()
...
def more_page(db):
        ...
        with st.form("sales"):
            ...
            if st.form_submit_button("Salvar vendedor") and n:
                if any(normalizar(v.get("email", "")) == normalizar(em) and em for v in db["salespeople"]):
                    ...
                    save_db(db); aviso("Vendedor salvo"); st.rerun()
    ...
    if st.button("Salvar comissão", key="btn_salvar_comissao"):
        db["commissionRate"] = rate; save_db(db); aviso("Comissão salva"); st.rerun()
    ...
    if up and st.button("Importar agora", key="btn_importar_backup"):
        new = json.loads(up.read().decode("utf-8")); save_db(new); aviso("Backup importado"); st.rerun()
    col1, col2 = st.columns(2)
    if col1.button("Restaurar exemplo", key="btn_restaurar_exemplo"):
        save_db(seed()); aviso("Exemplo restaurado"); st.rerun()
