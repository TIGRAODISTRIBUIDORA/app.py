...
def new_order(db):
 ...
 with st.form('order_form'):
  c=st.selectbox('Cliente', clients, format_func=lambda x:f"{x['name']} — {x['document']}")
  if st.session_state.role=='admin': s=st.selectbox('Vendedor', [x for x in sales if x.get('active')], format_func=lambda x:x['name'])
...
def orders_page(db):
 ...
 busca=st.text_input('Pesquisar pedido, cliente ou vendedor')
...
def clients_page(db):
 ...
 with st.expander('Cadastrar cliente'):
  with st.form('cli'):
   n=st.text_input('Nome'); doc=st.text_input('CPF/CNPJ'); em=st.text_input('E-mail'); ph=st.text_input('Telefone'); city=st.text_input('Cidade'); uf=st.text_input('Estado')
 ...
 busca=st.text_input('Pesquisar por nome, documento, cidade ou iniciais')
 for c in db['clients']:
...
def products_page(db):
 ...
 if st.session_state.role=='admin':
  with st.expander('Cadastrar produto'):
   with st.form('prod'):
    sku=st.text_input('Código/SKU'); n=st.text_input('Nome'); cat=st.text_input('Categoria'); price=st.number_input('Preço',min_value=0.0,step=.01); stock=st.number_input('Estoque',min_value=0,step=1); cr=st.number_input('Comissão %',min_value=0.0,value=float(db['commissionRate']),step=.5)
 ...
 busca=st.text_input('Pesquisar produto por código, nome ou categoria')
 for p in db['products']:
...
def more_page(db):
 ...
 with st.expander('Cadastrar vendedor'):
  with st.form('sales'):
   n=st.text_input('Nome'); em=st.text_input('E-mail'); cpf=st.text_input('CPF'); ph=st.text_input('Telefone'); pw=st.text_input('Senha',value='123')
 ...
 rate=st.number_input('Comissão geral %',min_value=0.0,value=float(db['commissionRate']),step=.5)
 if st.button('Salvar comissão'): db['commissionRate']=rate; save_db(db); st.success('Salvo')
 st.markdown('### Backup / Importação')
 st.download_button('Baixar backup JSON', data=json.dumps(db,ensure_ascii=False,indent=2).encode('utf-8'), file_name='tigrao_backup.json')
 ...
 st.download_button('Exportar Excel', data=out.getvalue(), file_name='tigrao_export.xlsx')
 up=st.file_uploader('Importar backup JSON', type=['json'])
 if up and st.button('Importar agora'):
  new=json.loads(up.read().decode('utf-8')); save_db(new); st.success('Importado'); st.rerun()
 col1,col2=st.columns(2)
 if col1.button('Restaurar exemplo'): save_db(seed()); st.rerun()
 if col2.button('Limpar banco'): save_db({'app':'Tigrão Distribuidora','systemName':'TIGRÃO','commissionRate':7,'products':[],'clients':[],'salespeople':INITIAL_SALES[:1],'orders':[]}); st.rerun()
